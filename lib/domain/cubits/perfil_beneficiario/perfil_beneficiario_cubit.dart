...
class PerfilBeneficiarioCubit extends Cubit<PerfilBeneficiarioState> {
  ...
  void getPerfilBeneficiarioDB(String id) async {
    final result =
        await perfilBeneficiarioDB.getPerfilBeneficiarioUsecaseDB(id);
    result.fold(
  ...
  void changeActivo(bool? value) {
    final tipoTenenciaChanged =
        state.perfilBeneficiario!.copyWith(tipoTenenciaId: value.toString());
    emit(PerfilBeneficiarioLoaded(tipoTenenciaChanged));
  }
